// Based on Markdown, adopted for mdr files
...
CodeMirror.defineMode("mdr", function(cmCfg, modeCfg) {
  ...
  const codeBlockLangRe = new RegExp(/{(.*?)}/);
  ...
  function inlineNormal(stream, state) {
    ...
    if (modeCfg.xml && ch === '<' && stream.match(/^(!--|\?|!\[CDATA\[|[a-z][a-z0-9-]*(?:\s+[a-z_:.\-]+(?:\s*=\s*[^>]+)?)*\s*(?:>|$))/i, false)) {
      ...
      if (end != -1) {
        ...
        if (/mdr\s*=\s*('|"){0,1}1('|"){0,1}/.test(atts)) state.md_inside = true;
      }
      stream.backUp(1);
...
CodeMirror.defineMIME("text/mdr", "mdr");

CodeMirror.defineMIME("text/x-mdr", "mdr");
