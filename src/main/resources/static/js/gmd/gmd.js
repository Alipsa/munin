// Based on Markdown, adopted for gmd files
...
CodeMirror.defineMode("gmd", function(cmCfg, modeCfg) {
  ...
  function inlineNormal(stream, state) {
    ...
    if (modeCfg.xml && ch === '<' && stream.match(/^(!--|\?|!\[CDATA\[|[a-z][a-z0-9-]*(?:\s+[a-z_:.\-]+(?:\s*=\s*[^>]+)?)*\s*(?:>|$))/i, false)) {
      ...
      if (end != -1) {
        ...
        if (/gmd\s*=\s*('|"){0,1}1('|"){0,1}/.test(atts)) state.md_inside = true;
      }
      stream.backUp(1);
...
CodeMirror.defineMIME("text/gmd", "gmd");

CodeMirror.defineMIME("text/x-gmd", "gmd");
